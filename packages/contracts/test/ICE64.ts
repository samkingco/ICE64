  ...
  describe("metadata", () => {
    ...
    describe("setOriginalsBaseURI", () => {
      it("sets the correct baseURI for originals", async () => {
        await contract.connect(owner).setOriginalsBaseURI("ar://<CID>");
        expect(await contract.getOriginalsBaseURI()).to.equal("ar://<CID>");
      });

      it("only lets the owner set the baseURI for originals", async () => {
        await expect(
          contract.connect(buyerA).setOriginalsBaseURI("ar://<CID>")
        ).to.be.revertedWith("UNAUTHORIZED");
      });
    });
    ...
    describe("storeChunkedEditionPhotoData", () => {
      it("can store edition photo data", async () => {
        ...
        for (const [index, image] of tokenDataArr
          ...
          .entries()) {
          expect(await contract.getRawEditionPhotoData(index + 1)).to.equal(
            image
          );
        }
      });
      ...
      it("only lets the owner store edition photo data", async () => {
        await expect(
          contract.connect(buyerA).storeChunkedEditionPhotoData(1, chunks[0])
        ).to.be.revertedWith("UNAUTHORIZED");
      });
    });
  });
    ...
    describe("originals", () => {
      describe("purchaseOriginal", () => {
        it("allows purchasing of originals", async () => {
          ...
          expect(endingBalance).to.eql([BigNumber.from(1), BigNumber.from(1)]);
        });
    ...
    describe("editions", () => {
      describe("purchaseEdition", () => {
        ...
        it("prevents purchasing an invalid edition", async () => {
          ...
          await expect(
            contract.connect(buyerB).purchaseEdition(0, { value: priceEdition })
          ).to.be.revertedWith("InvalidToken()");
        ...
        it("prevents exceeding max number of editions when original hasn't been purchased", async () => {
          ...
          await expect(
            contract
              .connect(buyerA)
              .purchaseEdition(originalId, { value: priceEdition })
          ).to.be.revertedWith("EditionForOriginalHolderStillReserved()");
      ...
      describe("getRawEditionPhotoData", () => {
        it("should get the raw photo data", async () => {
          expect(await contract.getRawEditionPhotoData(1)).to.equal(data[1]);
        });
      });

      describe.skip("getEditionPhotoSVG", () => {
        it("should get the photo in SVG format", async () => {
          expect(await contract.getEditionPhotoSVG(1)).to.eql(svgs[1]);
        });
      });
    ...
    describe("roots claiming", () => {
      ...
      describe("claimEditionAsRootsHolder", () => {
        ...
        it("should allow roots holders with multiple tokens to claim multiple editions", async () => {
          ...
          await contract.connect(rootsHolder).claimEditionAsRootsHolder(1, 10);
          await contract.connect(rootsHolder).claimEditionAsRootsHolder(8, 20);
          await contract.connect(rootsHolder).claimEditionAsRootsHolder(16, 30);
        ...
        it("prevents double claiming for roots holders", async () => {
          ...
          await expect(
            contract.connect(rootsHolder).claimEditionAsRootsHolder(2, rootsId)
